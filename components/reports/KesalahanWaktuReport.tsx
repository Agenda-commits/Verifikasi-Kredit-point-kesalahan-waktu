...
const KesalahanWaktuReport: React.FC<Props> = ({ data }) => {
  ...
  return (
    <div className="flex flex-col h-full animate-in space-y-5 max-h-full overflow-hidden">
      {/* Header Laporan */}
      ...
      <div className="grid grid-cols-12 gap-5 shrink-0">
        <div className="col-span-4 bg-white border border-zinc-200 shadow-sm p-6 flex flex-col justify-center min-h-[140px] rounded-sm">
          <div className="flex items-center gap-2 text-red-600 mb-2">
            <Activity size={12} />
            <span className="text-[8px] font-black tracking-[0.3em] uppercase italic">DEVIASI SINKRONISASI</span>
          </div>
          <div className="flex items-center gap-4">
            <h1 className="text-[54px] font-black text-black tracking-tighter leading-none italic">
              {data.deviasiWaktu.split(' ')[0]}
            </h1>
            <div className="flex flex-col items-start">
              <span className="text-base font-black text-black uppercase leading-none">{data.deviasiWaktu.split(' ')[1] || 'MENIT'}</span>
        ...
        <div className="col-span-4 bg-white border border-zinc-200 p-6 flex flex-col justify-center gap-2 rounded-sm shadow-sm">
        ...
        <div className="col-span-4 bg-zinc-50 border border-zinc-200 p-6 flex flex-col justify-center gap-1.5 rounded-sm shadow-sm">
            <div className="flex justify-between items-center pb-1 border-b border-zinc-200">
               ...
               <p className="text-[12px] font-black text-black italic uppercase">RP {formatClean(data.saldoAkunKerja)}</p>
            </div>
            <div className="flex justify-between items-center pb-1 border-b border-zinc-200">
               <p className="text-[7px] text-zinc-500 font-bold uppercase tracking-widest">BIAYA PEMULIHAN</p>
               <p className="text-[12px] font-black text-black italic uppercase">RP {formatClean(data.moneyIn)}</p>
            </div>
            <div className="flex justify-between items-center py-1">
               <p className="text-[7px] text-amber-700 font-black uppercase italic tracking-widest">KOMISI VALIDASI (50%)</p>
               <p className="text-[12px] font-black text-amber-600 italic uppercase">+ RP {formatClean(bonusKomisi)}</p>
            </div>
            <div className="pt-1.5 mt-1 border-t border-dashed border-zinc-300 flex justify-between items-end">
               <p className="text-[7px] text-emerald-800 font-black uppercase italic tracking-[0.2em] mb-1">TOTAL PENARIKAN AKHIR</p>
               <p className="text-[32px] font-black tracking-tighter text-emerald-700 leading-none italic uppercase">
      ...
      {/* Bagian Keterangan Analisa Teknis (Presisi & Rapi) */}
      <div className="bg-[#fffbeb] border border-[#fde047] flex-1 overflow-hidden min-h-0 rounded-sm shadow-sm flex items-stretch">
        {/* Ikon Alert Kiri */}
        <div className="w-16 shrink-0 flex items-center justify-center border-r border-amber-200/40 bg-amber-50/10">
           <AlertTriangle size={28} className="text-[#fbbf24]" strokeWidth={1.5} />
        </div>

        {/* Konten Narasi Tengah (Dibuat lebih fokus agar tidak terlalu lebar) */}
        <div className="flex-1 p-10 flex flex-col overflow-hidden">
          ...
          <div className="overflow-y-auto custom-scrollbar pr-10 flex-1">
             <div className={`${getFontSize(data.keteranganWaktu)} font-roboto-medium text-[#78350f] text-justify leading-relaxed`}>
                <span className="font-black block mb-2 text-[13px] tracking-tight uppercase">Diagnosa Sistem:</span>
        ...
        {/* Status Panel Kanan (Presisi & Rapi) */}
        <div className="w-[240px] shrink-0 border-l border-amber-300/40 bg-amber-50/20 p-10 flex flex-col justify-center gap-8">
           <div className="space-y-1">
              <p className="text-[7px] font-black text-amber-600 uppercase tracking-[0.2em] italic">SYNC STATUS</p>
              <p className="text-[13px] font-black text-amber-900 uppercase tracking-widest leading-none">IN-PROGRESS</p>
           </div>
           <div className="space-y-1">
              <p className="text-[7px] font-black text-amber-600 uppercase tracking-[0.2em] italic">ACCESS MODE</p>
              <p className="text-[13px] font-black text-zinc-800 uppercase italic tracking-widest leading-none">RESTRICTED</p>
           </div>
           <div className="mt-4 pt-6 border-t border-amber-300/30">
      ...
      {/* Footer Audit */}
      <div className="bg-black text-white px-6 py-4 flex items-center justify-between rounded-sm shrink-0">
         <div className="flex items-center gap-3">
            <div className="flex items-center justify-center p-1 bg-emerald-500/10 rounded">
               <ShieldCheck size={14} className="text-emerald-500" />
            </div>
            <span className="text-[9px] font-black tracking-[0.25em] uppercase italic">AUDIT TRAIL LOG: #SEC-NODE-X9 / TLS 1.3 ENCRYPTED PROTOCOL</span>
         </div>
         <span className="text-[8px] font-mono-tech text-zinc-500 italic uppercase tracking-[0.3em]">VX-992-AUTH-MILANO-GLOBAL</span>
      </div>
    </div>
  );
};
