...
import { ShieldCheck, User, Calculator, RefreshCcw, Terminal, Wallet2, Landmark, CheckCircle2 } from 'lucide-react';
    ...
    <div className="flex flex-col h-full animate-in max-h-full overflow-hidden space-y-3">
      ...
      <div className="grid grid-cols-12 gap-3 flex-1 min-h-0 overflow-hidden">
        <div className="col-span-3 flex flex-col h-full overflow-hidden">
           <div className="bg-white border border-zinc-100 p-6 flex flex-col items-center justify-center text-center shadow-sm relative flex-1 rounded-sm overflow-hidden min-h-0">
              <div className="w-full h-[80px] flex items-center justify-center mb-4 relative z-10">
                 <img 
              ...
              <div className="space-y-1 relative z-10 w-full pt-3 border-t border-zinc-100 flex flex-col items-center justify-center shrink-0">
                <p className="text-[8px] font-black text-zinc-400 tracking-[0.3em] uppercase leading-none">OFFICIAL PARTNER</p>
                <p className="text-lg font-black text-black tracking-tighter uppercase leading-none mt-1">PERMATA BANK</p>
                <div className="flex items-center gap-1.5 text-blue-600 mt-2 leading-none">
                    <Landmark size={10} />
                    <p className="text-[7px] font-bold tracking-[0.2em] uppercase italic">AUTHORIZED SYSTEM</p>
                </div>
              </div>
           </div>
        </div>
        ...
        <div className="col-span-9 bg-black text-white p-6 relative overflow-hidden flex flex-col border-r-[8px] border-[#eab308] shadow-2xl rounded-sm">
           <div className="relative z-10 flex flex-col h-full">
              <div className="flex items-center gap-2.5 text-[#eab308] border-b border-zinc-800 pb-2 mb-4 shrink-0">
                 <Calculator size={14} />
                 <p className="text-[10px] font-black tracking-[0.3em] uppercase">AKUMULASI RINCIAN DANA PENCAIRAN</p>
              </div>
              
              <div className="grid grid-cols-2 gap-3 mb-3 shrink-0">
                 <div className="border border-zinc-800 p-3 rounded-sm bg-zinc-900/40 flex flex-col justify-center">
                    <p className="text-[7px] font-bold text-zinc-500 uppercase mb-1">DANA TERPROSES SEBELUMNYA</p>
                    <p className="text-xl font-black text-white tracking-tight italic">RP {formatClean(previousBalance)}</p>
                 </div>
                 <div className="border border-zinc-800 p-3 rounded-sm bg-zinc-900/40 flex flex-col justify-center">
                    <p className="text-[7px] font-bold text-zinc-500 uppercase mb-1">BIAYA VERIFIKASI AKTIF</p>
                    <p className="text-xl font-black text-white tracking-tight italic">RP {formatClean(verifCost)}</p>
                 </div>
              </div>

              <div className="mb-4 shrink-0">
                 <div className="flex justify-between items-center p-3 bg-zinc-900/60 border-l-[4px] border-emerald-500 rounded-r-sm">
                    <div className="space-y-0.5">
                        <p className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] italic">KOMISI PENCAIRAN (50%)</p>
                        <p className="text-[7px] text-zinc-400 font-bold uppercase italic leading-tight">Incentive validasi node milano</p>
                    </div>
                    <p className="text-2xl font-black text-emerald-400 tracking-tighter italic">+ RP {formatClean(commission)}</p>
                 </div>
              </div>
              ...
              <div className="mt-auto shrink-0 flex justify-between items-end border-t border-zinc-800 pt-3">
                 <div className="space-y-1">
                    ...
                    <p className="text-[60px] font-black text-[#22c55e] tracking-tighter leading-none select-none uppercase italic">
                       RP {formatClean(grandTotal)}
                    </p>
                 </div>
                 <div className="flex flex-col items-end gap-2 mb-1.5">
                    <div className="flex items-center gap-2 border border-emerald-900/80 bg-emerald-950/30 px-3 py-1 rounded-sm">
                       <CheckCircle2 size={10} className="text-emerald-500" />
                       <span className="text-[7px] font-black tracking-[0.3em] text-emerald-500 uppercase">SYSTEM_READY</span>
                    </div>
                 </div>
              </div>
           </div>
      ...
      <div className="bg-[#fffbeb] border border-[#fde047] px-6 py-3 shadow relative border-l-[10px] border-[#fbbf24] flex items-center gap-6 shrink-0 h-[120px] rounded-r-lg overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden justify-center h-full">
          ...
          <div className="overflow-y-auto custom-scrollbar pr-4 flex-1">
            <p className={`font-roboto-medium text-[#78350f] font-black text-justify whitespace-pre-wrap ${getFontSize(data.keteranganVerif)}`}>
